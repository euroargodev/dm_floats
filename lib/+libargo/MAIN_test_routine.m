...
index_file_name='/home5/pharos/REFERENCE_DATA/ARGO/DATA/ArgoData_last/argo_profile_detailled_index.txt';
...
[liste_profile_files,liste_floats_files] = select_floats_from_index(index_data,'lonmin',-70, 'lonmax', 10, 'latmin',30, 'latmax', 70 ,'ocean','A','exclude_type',{'852'},'data_mode','D');
...
DIRFTP = '/home5/pharos/REFERENCE_DATA/ARGO/DATA/ArgoData_last/';
...
[FL,Dim,Globatt]=read_netcdf_allthefile(FILENAME);
...
FL2 = read_netcdf_allthefile(FILENAME,Param);
% ou
[FL2, Dim2] = read_netcdf_allthefile(FILENAME,Param);
...
%--------------------------------------------------------------------------------------------
isallFillValue = check_isfillval_prof(FL,'psal');
...
FL_best = construct_best_param(FL,{'psal','pres','temp'})
...
subplot(1,2,1)
thetitle = plot_profile_with_flag(FL,'temp','pres',nprof);

subplot(1,2,2)
plot_profile_with_flag(FL,'psal','pres',nprof);
...
plot_traj_atlantique(FL)

% tracer les salinites du premier niveau le long de la trajectoire
[thetitle] = plot_traj_atlantique(FL, FL.psal.data(:,1))
...
[FL_ex,Dim_ex] = remove_profile_dim(FL,Dim,'N_PROF',1);
...
[FL_ex,Dim_ex] = extract_profile_dim(FL,Dim,'N_PROF',3);


% extrait les profils nprof=[3:10];
[FL_ex,Dim_ex] = extract_profile_dim(FL,Dim,'N_PROF',[3:10]);

% extrait le premier niveau vertical n_levels=1 de chaque profil;
[FL_surf,Dim_surf] = extract_profile_dim(FL,Dim,'N_LEVELS',[1]);
...
FL_ex = check_FirstDimArray_is(FL_ex,'N_HISTORY')

% cree le fichier netcdf
create_netcdf_allthefile(FL_ex,Dim_ex,'toto.nc',Globatt)
%ou
create_netcdf_allthefile(FL_ex,Dim_ex,'toto.nc') % sans les attributs globaux


% Concatene deux fichiers
%--------------------------------------------------------------------------------------------
[FL1,Dim1] = read_netcdf_allthefile([DIRFTP 'coriolis/5902269/profiles/D5902269_001.nc']);
[FL2,Dim2] = read_netcdf_allthefile([DIRFTP 'coriolis/5902269/profiles/D5902269_002.nc']);


[FL12,Dim12]=cat_profile_dim(FL1,FL2,Dim1,Dim2,'N_PROF'); % on concatene selon la dimension N_PROF

% verifie que la premiere dimension des tableaux est bien N_HISTORY

FL12 = check_FirstDimArray_is(FL12,'N_HISTORY')

% cree le fichier netcdf
create_netcdf_allthefile(FL12,Dim12,'toto.nc',Globatt)
...
print_history([DIRFTP 'coriolis/5902269/profiles/D5902269_001.nc'])


% trace les pressions de surface pour un flotteur
%%--------------------------------------------------------------------------------------------

[F] = plot_surface_pres('5902269','coriolis',DIRFTP)
